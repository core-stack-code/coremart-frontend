...
const ProductGridCard: React.FC<ProductCardGridProps> = ({ item, cardType }) => {
      ...
      <div className="w-64 flex-shrink-0  overflow-hidden border-border border rounded-[var(--border-radius)] bg-white relative">
...

        {
          cardType === "NewArrivals" || cardType === "TrendingOffer" || cardType === "TrendingProduct" ?
            <Badge variant="outline" className='absolute bg-primary top-2 left-2 border-none text-background px-3 rounded-2xl'>New</Badge> : null
        }


        {/* For Image and naviagte */}

        <NavLink to={`/product/${product.slug}`} className="w-full h-64  flex justify-center items-center">
        ...
        <div className="p-4 space-y-2">
...

          {
            cardType === 'RecommendedRecent' ?
              <div className="flex justify-between items-center">
                <span className="font-semibold text-lg text-muted">₹ {product.price}</span>
