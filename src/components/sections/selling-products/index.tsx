...
const SellingProducts: React.FC = () => {
    ...
    return (
        <div className='flex flex-col justify-center items-center py-10 px-24 gap-10 rounded-rad bg-surface/70'>
            ...
            <ProductGrid columns={4} rows={1} className='gap-5'>
                {data?.data?.products.map((product) => <ProductCard key={product.id} product={product} cardType="ratting" isRedirect />)}
            </ProductGrid>
        </div>
    )
}
